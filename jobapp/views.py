...
# load the fine-tuned BERT model for job recommendation
model = pipeline('text2text-generation', model='path/to/fine-tuned/model')

@csrf_exempt
def search(request):
    if request.method == 'POST':
        query = request.POST.get('query')
        # use the fine-tuned BERT model to generate job recommendations
        results = model(query, max_length=20, num_return_sequences=5)
        # return the top 5 job recommendations as search results
...
# load the search model and tokenizer
model_name = "nboost/pt-bert-base-uncased-msmarco"
tokenizer = AutoTokenizer.from_pretrained(model_name)
model = AutoModelForSequenceClassification.from_pretrained(model_name)

# define a function to perform the search
def search_model(query, job_descriptions, top_k=5):
    # encode the query and job descriptions
    query_encoding = tokenizer(query, padding=True, truncation=True, return_tensors="pt")
    jd_encodings = tokenizer(job_descriptions, padding=True, truncation=True, return_tensors="pt", max_length=512)

    # get the logits (scores) for the query and job descriptions
    logits = model(**query_encoding, input_ids=jd_encodings["input_ids"], attention_mask=jd_encodings["attention_mask"]).logits

    # apply softmax to convert logits to probabilities
    probs = logits.softmax(dim=1)

    # sort the probabilities in descending order and return the top k indices
    top_k_indices = probs[0].argsort(descending=True)[:top_k].tolist()

    # return the job descriptions corresponding to the top k indices
    return [job_descriptions[i] for i in top_k_indices]

def search(request):
    if request.method == 'POST':
        query = request.POST.get('query')
        job_descriptions = [jd.JobDescription for jd in JobDescription.objects.all()] # replace this with your own way of getting job descriptions
        results = search_model(query, job_descriptions)
